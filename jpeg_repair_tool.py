...
class MainWindow(QMainWindow):
    ...
    # ======================================================================
    # --- SINGLE-FILE REPAIR LOGIC ---
    # ======================================================================
    def get_repair_filepaths(self):
        """
        Determines normalized absolute input and output paths for single-file operation.
        """
        ...
        output_file = os.path.normpath(os.path.abspath(os.path.join(repaired_dir, original_filename)))[cite: 2]
        input_file = os.path.normpath(os.path.abspath(self.current_filepath))[cite: 2]
        
        return input_file, output_file[cite: 1, 2]

    def execute_jpegrepair(self, command, operation):
        script_dir = os.path.dirname(os.path.abspath(__file__))[cite: 1, 2]
        exe_path = os.path.normpath(os.path.join(script_dir, "jpegrepair.exe"))[cite: 1, 2]

        if not os.path.exists(exe_path):[cite: 1, 2]
            return False, f"Executable not found: {exe_path}. Ensure it is in the same folder as this script."[cite: 1, 2]
        
        command.insert(0, exe_path)[cite: 1, 2]
        
        try:
            process = subprocess.run(command, capture_output=True, text=True, check=False)[cite: 1, 2]
            if process.returncode == 0:[cite: 1, 2]
                return True, ""[cite: 1, 2]
            else:
                error_output = process.stderr if process.stderr else "No specific error output."[cite: 1, 2]
                return False, f"Execution failed for {operation} with return code {process.returncode}.\nError:\n{error_output}"[cite: 1, 2]

        except Exception as e:[cite: 1, 2]
            return False, f"An unexpected error occurred during execution: {e}"[cite: 1, 2]
