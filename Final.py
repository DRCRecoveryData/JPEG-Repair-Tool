...
def count_gray_mcu_scanlines(filepath, mcu_data, gray_tolerance=10, color_std_dev_threshold=5):
    ...
    try:
        ...
        for i in range(total_mcu_scanlines):
            ...
            # Optimization: Stop counting if the gray blocks are not contiguous at the end
            elif gray_scanline_count > 0:
                 # If we found gray, but now found a non-gray, we break, 
                 # as only contiguous footer gray blocks can be cropped
                 break
...
def crop_jpeg_by_header(source_filepath, output_filepath, scanlines_to_remove):
    ...
    try:
        # NOTE: shutil.copy2 handles overwrite, which is required for this new file naming scheme
...
# ======================================================================
# --- PyQt GUI Components (Classes updated for modern look) ---
# ======================================================================
...
class McuGridItem(QGraphicsItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        # Grid color changed to a subtle dark theme white/blue
        ...
        # Hovered block fill/border (used a dark theme blue)
        if self.hovered_mcu_coords:
            hr, hc = self.hovered_mcu_coords
            hx = hc * self.mcu_x
            hy = hr * self.mcu_y
            h_rect = QRectF(hx, hy, self.mcu_x, self.mcu_y)
            painter.fillRect(h_rect, QColor(0, 119, 255, 60)) # Blue transparency
        
        # Selected block fill/border (used a darker theme red for error/selection)
        if self.selected_mcu_coords:
            ...
            painter.fillRect(highlight_rect, QColor(255, 60, 60, 70)) # Red transparency
            painter.setPen(QPen(QColor(255, 60, 60), 3)) # Red border
...
class McuGraphicsView(QGraphicsView):
    # Class logic remains the same, appearance is handled by QSS.
...
class BlockPreviewWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.is_static_preview and self.is_hovered:
            painter.setPen(QPen(QColor(0, 119, 255), 2)) # Accent Blue
            painter.drawRect(0, 0, self.width()-1, self.height()-1)
        elif not self.is_static_preview and self.is_selected:
            painter.setPen(QPen(QColor(255, 60, 60), 2)) # Accent Red
            painter.drawRect(0, 0, self.width()-1, self.height()-1)
        else:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.vertical_gray_scanlines_to_remove = 0 # NEW: Store the calculated count
    ...
    # ======================================================================
    # --- File Path Logic (MODIFIED for Overwrite) ---
    # ======================================================================
    def get_repair_filepaths(self):
        ...
        # The output file is always Repaired/Original_Filename and will overwrite any previous repair.
        output_file = os.path.join(repaired_dir, original_filename)
        
        # The input file is the currently displayed file (either original or previous repair)
    ...
    def remove_gray_scanlines(self):
        ...
        if success:
            ...
            # IMPORTANT: Reload the view using the newly cropped file.
            self.load_and_display_image(output_file) 
            
            # Post-crop check is run inside load_and_display_image
    ...
    def run_auto_alignment(self):
        ...
        # Blocks to insert is always 1 + the number of gray MCUs remaining (to fill the partial line)
        ...
        c = 0
        r = 0 # MCU destination coord for insertion
        
        # CALL MODIFIED
        input_file, output_file = self.get_repair_filepaths() 
        if not input_file: return
        
        # This insertion is always done at the file header (0, 0) for alignment
        command = [
            input_file,
            output_file,
            "dest",
    ...
    def run_auto_color_correction(self):
        ...
        try:
            ...
            # Use original file extension and save to the determined output_file (overwriting)
    ...
    # --- Run CDelta Repair Method (MODIFIED for Overwrite/Temp File) ---
    def run_cdelta_repair(self):
        ...
        try:
            for comp_index in active_components:
                ...
                # If last component, output to the final, non-versioned name (overwriting previous repairs)
                # Otherwise, output to the unique temp file.
